...
class EquidistantIonChain:
    def __init__(self, ion_type, n_ions, dist, nu_rad):
        self.n_ions = n_ions
        self.nu_rad = nu_rad
        self.omega_ax  = 2*math.pi*nu_ax
        self.omega_rad = 2*math.pi*nu_rad
...
class EquidistantIonChain:
    def __init__(self, ion_type, n_ions, dist, nu_rad):
        ...
        self.omega_rad = 2*math.pi*nu_rad
        e = 1e-5
        self.eq_pos = np.arange(-(n_ions - 1)/2, (n_ions-1)/2 + e)*dist
        
        omega_dx = math.sqrt(sconst.e**2/(4*math.pi*sconst.epsilon_0*self.ion.m*dist**3))

        self.omegas_ax,  self.normal_modes_ax  =  get_axial_equidistant_normal_modes(omega_dx, n_ions)
        self.omegas_rad, self.normal_modes_rad = get_radial_equidistant_normal_modes(omega_dx, self.omega_rad, n_ions)
